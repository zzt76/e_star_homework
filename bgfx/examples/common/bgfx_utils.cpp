...
bgfx::TextureHandle
loadTexture(bx::FileReaderI *_reader, const char *_filePath, uint64_t _flags, uint8_t _skip, bgfx::TextureInfo *_info,
        ...
        if (NULL != imageContainer) {
            ...
            if (NULL != _info) {
                bgfx::calcTextureSize(
                        *_info, uint16_t(imageContainer->m_width), uint16_t(imageContainer->m_height),
    ...
    while (4 == bx::read(_reader, chunk, &err)
        ...
        switch (chunk) {
            ...
            case kChunkIndexBufferCompressed: {
                ...
                meshopt_decodeIndexBuffer(mem->data, group.m_numIndices, 2, (uint8_t *) compressedIndices,
                                          compressedSize);
...
void Mesh::submit(bgfx::ViewId _id, bgfx::ProgramHandle _program, const float *_mtx, uint64_t _state) const {
    ...
    for (GroupArray::const_iterator it = m_groups.begin(), itEnd = m_groups.end(); it != itEnd; ++it) {
        ...
        bgfx::submit(
                _id, _program, 0, BGFX_DISCARD_INDEX_BUFFER
...
void Mesh::submit(const MeshState *const *_state, uint8_t _numPasses, const float *_mtx, uint16_t _numMatrices) const {
    ...
    for (uint32_t pass = 0; pass < _numPasses; ++pass) {
        ...
        for (GroupArray::const_iterator it = m_groups.begin(), itEnd = m_groups.end(); it != itEnd; ++it) {
            ...
            bgfx::setVertexBuffer(0, group.m_vbh);
            bgfx::submit(
                    state.m_viewId, state.m_program, 0, BGFX_DISCARD_INDEX_BUFFER
                                                        | BGFX_DISCARD_VERTEX_STREAMS
            );
        }
...
void meshSubmit(const Mesh *_mesh, const MeshState *const *_state, uint8_t _numPasses, const float *_mtx,
                uint16_t _numMatrices) {
