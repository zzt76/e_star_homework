...
namespace imgui_addons {
    ...
    bool ImGuiFileBrowser::showFileDialog(const std::string &label, const DialogMode mode, const ImVec2 &sz_xy,
        ...
        ImGui::SetNextWindowSize(ImVec2(std::max<float>(sz_xy.x, min_size.x), std::max<float>(sz_xy.y, min_size.y)),
                                 ImGuiCond_Appearing);
    ...
    bool ImGuiFileBrowser::renderFileListRegion() {
        ...
        int num_cols = static_cast<int>(std::max<float>(1.0f, std::ceil(
        ...
        if (num_cols > 64) {
            ...
            num_cols = static_cast<int>(std::max<float>(1.0f, std::ceil(
                    static_cast<float>(filtered_dirs.size() + filtered_files.size()) / col_items_limit)));
        }
